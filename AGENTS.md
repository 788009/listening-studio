...
- 每个话轮可以独立生成和预览，也可以重新生成。长文本按 TTS 切割规则展示为多个独立预览段，每段可以单独重新生成；整话轮重新生成会按当前全文重新切割。最终按钮先为所有未生成话轮生成预览，全部完成后才允许发布。
...
- 每个批量生成任务只接收一种题型及其数量，并接收语料和说话人到音色的映射。短对话和长对话至少需要两个说话人，独白至少需要一个说话人。
